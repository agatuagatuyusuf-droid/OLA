...
namespace SkyAuto.Core.Steps.LogSteps;

public class WriteLogExecutor : ActionStepExecutorBase
{
    public override string Type => "write_log";

    protected override async Task<string?> DoExecuteAsync(WorkflowStep step, Dictionary<string, object?>? ctx)
    {
        var msg = step.Params.GetValueOrDefault("message")?.ToString() ?? "";
        var logDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "logs");
        Directory.CreateDirectory(logDir);
        var logPath = Path.Combine(logDir, "app.log");
        await File.AppendAllTextAsync(logPath, $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} - {msg}\n");
        return $"已记录日志";
    }
}

public class SaveScreenshotExecutor : ActionStepExecutorBase
{
    public override string Type => "save_screenshot";

    protected override async Task<string?> DoExecuteAsync(WorkflowStep step, Dictionary<string, object?>? ctx)
    {
        var path = step.Params.GetValueOrDefault("path")?.ToString() ?? $"data/screenshots/screenshot_{DateTime.Now:yyyyMMddHHmmss}.bmp";
